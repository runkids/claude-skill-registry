---
name: command-phase-report
description: Use the /phase-report slash command to generate progress updates and log them. Use when: reporting status or phase completion.
---

# Command: /phase-report

Run the `/phase-report` workflow defined in `.claude/commands/phase-report.md`.

## Usage
`/phase-report`

## Notes
- Appends reports to `.brain/context/current.md`.
- Full workflow details: `.claude/commands/phase-report.md`.
