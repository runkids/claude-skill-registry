...
Azure Static Web Apps (SWA) hosts static frontends with optional serverless API
backends. The SWA CLI (`swa`) provides local development emulation and
...
- `swa-cli.config.json` - CLI settings, **created by `swa init`** (never create
  manually)
- `staticwebapp.config.json` - Runtime config (routes, auth, headers, API
  runtime) - can be created manually
...
**IMPORTANT: Always use `swa init` to create configuration files. Never manually
create `swa-cli.config.json`.**

1. `swa init` - **Required first step** - auto-detects framework and creates
   `swa-cli.config.json`
...
**staticwebapp.config.json** (in app source or output folder) - This file CAN be
created manually for runtime configuration:
...
**Flags:** `--subscription-id, -S` | `--resource-group, -R` | `--tenant-id, -T`
| `--client-id, -C` | `--client-secret, -CS` | `--app-name, -n`

### swa init

Configure a new SWA project based on an existing frontend and (optional) API.
...
**Flags:** `--app-location, -a` | `--api-location, -i` | `--output-location, -O`
| `--app-build-command, -A` | `--api-build-command, -I`
...
**Common framework ports:** | Framework | Port | |-----------|------| |
React/Vue/Next.js | 3000 | | Angular | 4200 | | Vite | 5173 |
...
**Always run `swa init` before `swa start` or `swa deploy`. Do not manually
create `swa-cli.config.json`.**
...
**Supported API runtimes:** `node:18`, `node:20`, `node:22`, `dotnet:8.0`,
`dotnet-isolated:8.0`, `python:3.10`, `python:3.11`
...
3. **Add secret:** Copy deployment token to repository secret
   `AZURE_STATIC_WEB_APPS_API_TOKEN`
