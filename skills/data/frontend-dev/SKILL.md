...
**CRITICAL:** All interactive components MUST have `data-testid` attributes for UI-Playwright testing.

### Required Attributes

```tsx
// Container with test ID and state
<div
...
>
  ...
  {data.map(item => (
    <div
      ...
      data-shootout-id={item.id}
      data-status={item.status}
    >
      {/* Actions with specific test IDs */}
      <button data-testid="shootout-card-delete-btn">
        Delete
      </button>
    </div>
  ))}
...
function MyList() {
  ...
  // Loading
  if (isLoading) {
    return <div data-testid="my-list" data-loading="true"><Skeleton /></div>;
  }

  // Error
  if (error) {
    return <div data-testid="my-list" data-error="true"><ErrorDisplay /></div>;
  }

  // Empty
  if (!data?.length) {
    return <div data-testid="my-list" data-empty="true"><EmptyState /></div>;
  }

  // Content
  return (
    <div data-testid="my-list" data-loading="false" data-count={data.length}>
      {data.map(...)}
    </div>
  );
}
...
Full documentation: [Frontend Development Standards](https://github.com/krazyuniks/guitar-tone-shootout/wiki/Frontend-Development-Standards)
