...
1. **Locate** - Find the prompt in its source file (Python, YAML, Markdown, etc.)
2. **Analyze** - Run automated checks + apply judgment checklist with visible reasoning
3. **Edit In-Place** - Modify the prompt directly in the source file
4. **Summarize** - Output a summary of changes made (no separate file needed)
</instructions>

<process>
## Step 0: Locate the Prompt
...
3. Note the exact location so you can edit it in-place later

---

## Step 1: Run Automated Checks

Execute the Python check scripts on the prompt content:
...
- Use the Edit tool to replace the original prompt with the improved version
- Preserve the surrounding code/structure (quotes, variable assignments, indentation, etc.)
- For multi-line strings, maintain proper escaping and formatting for the file type
...
<constraints>
## What You MUST Do
- Locate the prompt in its source file before analyzing
...
- Edit the prompt IN-PLACE in the source file (do not create separate output files)
- Output the summary directly in your response (do not create a summary file)
...
- Create separate output files (edit in-place instead)
