...
description: "Generate meta-prompts. Use when: Creating optimized prompts for Claude-to-Claude pipelines or multi-stage workflows. Not for: Simple single-turn prompts or manual messaging."
---
...
Each prompt gets its own folder in `.claude/planning/prompts/` with its output artifacts, enabling clear provenance and chain detection.
...
4. **Save**: Create folder in `.claude/planning/prompts/{number}-{topic}-{purpose}/`
...
Next prompt number: !`ls -d ./.claude/planning/prompts/*/ 2>/dev/null | wc -l | xargs -I {} expr {} + 1`
...
For Refine purpose, also identify target output from `.claude/planning/prompts/*/` to improve.
##### Chain Detection
Scan `.claude/planning/prompts/*/` for existing `*-research.md` and `*-plan.md` files.
If found:
...
Folder: .claude/planning/prompts/{number}-{topic}-{purpose}/
...
#### File Creation
1. Create folder: `.claude/planning/prompts/{number}-{topic}-{purpose}/`
2. Create `completed/` subfolder
3. Write prompt to: `.claude/planning/prompts/{number}-{topic}-{purpose}/{number}-{topic}-{purpose}.md`
4. Prompt instructs output to: `.claude/planning/prompts/{number}-{topic}-{purpose}/{topic}-{purpose}.md`
...
```
Prompt created: .claude/planning/prompts/{number}-{topic}-{purpose}/{number}-{topic}-{purpose}.md
What's next?
1. Run prompt now
...
```
Prompts created:
- .claude/planning/prompts/001-auth-research/001-auth-research.md
- .claude/planning/prompts/002-auth-plan/002-auth-plan.md
- .claude/planning/prompts/003-auth-implement/003-auth-implement.md
...
3. Include in task prompt:
   ...
   - Output location: `.claude/planning/prompts/{number}-{topic}-{purpose}/{topic}-{purpose}.md`
...
1. Parse each prompt for `@.claude/planning/prompts/{number}-{topic}/` patterns
...
2. Check if it exists in `.claude/planning/prompts/*/` (already completed)
3. If truly missing:
...
```bash
mv .claude/planning/prompts/{number}-{topic}-{purpose}/{number}-{topic}-{purpose}.md \
   .claude/planning/prompts/{number}-{topic}-{purpose}/completed/
```
...
✓ Created: .claude/planning/prompts/001-auth-research/SUMMARY.md
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
All prompts archived. Full summaries in .claude/planning/prompts/*/SUMMARY.md
What's next?
...
- Folder created in `.claude/planning/prompts/` with correct naming
