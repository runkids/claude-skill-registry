...
description: "Generate production-ready Row Level Security (RLS) policies for Supabase tables with is_super_admin() bypass, proper USING/WITH CHECK, automated testing framework, performance analysis, and coverage reports. Use when creating RLS policies, securing tables, testing access controls, analyzing policy performance, or auditing RLS coverage."
version: "3.0.0"
updated: "2026-01-12"
---
...
**Purpose**: Automated testing prevents 99% of RLS bugs by catching issues before production deployment.

### Why Test RLS Policies?

**Common RLS bugs caught by testing**:
- ✅ Policy allows unauthorized access (security vulnerability)
- ✅ Policy blocks legitimate access (usability bug)
- ✅ Policy has performance issues (EXPLAIN ANALYZE shows seq scans)
- ✅ Personal accounts blocked (missing primary_owner_user_id check)
- ✅ Super admin bypass missing
- ✅ Infinite recursion risk

**Testing prevents**:
- Production hotfixes (80% are RLS-related)
- User support tickets ("Can't access my data")
- Security incidents (data leaks)
- Performance degradation (slow queries)

### Test Pattern 1: Vitest RLS Tests (Recommended)

Use the dual-client pattern from `test-patterns` skill:

```typescript
// apps/web/__tests__/rls/productions.test.ts
import { describe, it, expect } from 'vitest';
import { getServiceRoleSupabaseClient, getSupabaseRouteHandlerClient } from '@kit/supabase/route-handler-client';
import { Database } from '@kit/supabase/database';

describe('Productions RLS Policies', () => {
  const adminClient = getServiceRoleSupabaseClient<Database>();

  it('super admin can read all productions', async () => {
    // Setup: Create test data with admin client
    const { data: production, error: insertError } = await adminClient
      .from('productions')
      .insert({
    ...
    const { data: superAdminUser } = await adminClient.auth.admin.listUsers();
    const superAdmin = superAdminUser.users.find((u) =>
      u.email?.includes('admin')
    );
    expect(superAdmin).toBeDefined();

    const userClient = await getSupabaseRouteHandlerClient<Database>({
      admin: false,
      user: superAdmin!,
    });
    ...
    expect(data?.id).toBe(production!.id);

    // Cleanup
    await adminClient.from('productions').delete().eq('id', production!.id);
  });

  it('regular user cannot read production they are not assigned to', async () => {
    // Setup: Create test production
  ...
  it('personal account owner can access their data', async () => {
    // Create personal account user
    ...
    // Get their auto-created personal account
    ...
    // Create data in their personal account
    const { data: production } = await adminClient
      .from('productions')
      .insert({
    ...
    // Test: Personal account owner can access
    const userClient = await getSupabaseRouteHandlerClient<Database>({
      admin: false,
      user: userData.user,
    });
...
### Test Pattern 2: Direct SQL Tests (Faster)

For quick validation during migration development:

```sql
-- Test script: test_rls_productions.sql
-- Run with: psql ... -f test_rls_productions.sql

BEGIN;

-- Setup: Create test users
INSERT INTO auth.users (id, email) VALUES
  ('11111111-1111-1111-1111-111111111111', 'super-admin@test.com'),
  ('22222222-2222-2222-2222-222222222222', 'regular-user@test.com')
ON CONFLICT (id) DO NOTHING;

-- Mark first user as super admin
INSERT INTO public.profiles (id, is_super_admin) VALUES
  ('11111111-1111-1111-1111-111111111111', true)
ON CONFLICT (id) DO UPDATE SET is_super_admin = true;

-- Setup: Create test production
INSERT INTO public.productions (id, name, client_id, status) VALUES
  ('aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaaa', 'Test Production', 'test-client', 'open');

-- TEST 1: Super admin can read
SET request.jwt.claims.sub = '11111111-1111-1111-1111-111111111111';
SELECT 'TEST 1 (super admin read):',
  CASE
...
-- TEST 2: Regular user cannot read (not assigned)
SET request.jwt.claims.sub = '22222222-2222-2222-2222-222222222222';
SELECT 'TEST 2 (regular user blocked):',
  CASE
    WHEN COUNT(*) = 0 THEN '✅ PASS'
    ELSE '❌ FAIL - User should not see production'
  END as result
FROM public.productions
WHERE id = 'aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaaa';

-- TEST 3: Regular user can insert (if allowed by policy)
INSERT INTO public.productions (name, client_id, status)
VALUES ('User Production', 'user-client', 'open');

SELECT 'TEST 3 (user insert):',
  CASE
    WHEN COUNT(*) >= 1 THEN '✅ PASS'
    ELSE '❌ FAIL - Insert blocked'
  END as result
FROM public.productions
WHERE name = 'User Production';
...
**Purpose**: Catch slow policies before they hit production.
...
-- ❌ "Seq Scan" - Missing index, will be slow at scale
-- ❌ "Subplan" with high cost - Policy might be inefficient
-- ❌ High "Execution Time" (> 100ms) - Slow policy
-- ✅ "Index Scan" - Good, using indexes
-- ✅ Low execution time (< 50ms) - Fast policy
...
```
❌ BAD (Sequential Scan):
Seq Scan on productions (cost=0.00..2345.00 rows=100 width=123)
  Filter: (status = 'open' AND (is_super_admin() OR ...))
  Rows Removed by Filter: 9900
Planning Time: 2.5 ms
Execution Time: 456.2 ms

✅ GOOD (Index Scan):
Index Scan using productions_status_idx (cost=0.29..123.45 rows=100 width=123)
  Index Cond: (status = 'open')
  Filter: (is_super_admin() OR ...)
...
```sql
-- performance_test_rls.sql
-- Tests all tables with RLS policies for performance issues
...
DECLARE
  ...
  slow_threshold NUMERIC := 100; -- milliseconds
BEGIN
  RAISE NOTICE 'Starting RLS Performance Audit...';
  RAISE NOTICE '';
  ...
  FOR table_record IN
    ...
    ORDER BY tablename
  LOOP
    RAISE NOTICE 'Testing table: %', table_record.tablename;

    -- Execute and capture EXPLAIN ANALYZE
    EXECUTE format('EXPLAIN (ANALYZE, FORMAT JSON) SELECT * FROM public.%I LIMIT 10',
      table_record.tablename)
    INTO explain_output;

    -- Extract execution time from JSON
    execution_time := (explain_output::json->0->'Execution Time')::numeric;

    -- Report result
    IF execution_time > slow_threshold THEN
      RAISE WARNING '  ❌ SLOW: %.2f ms (threshold: % ms)', execution_time, slow_threshold;
    ELSE
      RAISE NOTICE '  ✅ FAST: %.2f ms', execution_time;
    END IF;

    RAISE NOTICE '';
  END LOOP;

  RAISE NOTICE 'RLS Performance Audit Complete';
END $$;
```
...
```sql
-- coverage_report_rls.sql
-- Identifies tables missing RLS policies
...
SELECT
  ...
  CASE
    ...
    WHEN tp.tablename IS NULL THEN '⚠️ RLS Enabled but No Policies'
...
--------------------+---------------------------------+--------------
 legacy_table       | ❌ RLS Disabled                 |            0
 new_table          | ⚠️ RLS Enabled but No Policies  |            0
 productions        | ✅ RLS Enabled with Policies    |            9
 events             | ✅ RLS Enabled with Policies    |           16
```

### Policy Quality Audit

**Check for common policy mistakes**:

```sql
-- audit_policy_quality.sql
-- Identifies common RLS policy issues

-- Issue 1: Policies without is_super_admin() bypass
SELECT
  'Missing super admin bypass' as issue,
  tablename,
  policyname,
  cmd
FROM pg_policies
...
-- Issue 2: Policies with "Allow all" pattern (security risk)
SELECT
  'Allow all pattern detected' as issue,
  tablename,
  policyname,
  cmd,
  qual
...
-- Issue 3: Policies using accounts_memberships without personal account check
SELECT
  'Missing personal account check' as issue,
  tablename,
  policyname,
  cmd
FROM pg_policies
WHERE schemaname = 'public'
AND qual LIKE '%accounts_memberships%'
AND qual NOT LIKE '%primary_owner_user_id%'
ORDER BY tablename;

-- Issue 4: Policies with potential infinite recursion (EXISTS + JOIN)
SELECT
  'Potential recursion risk' as issue,
  tablename,
  policyname,
  cmd,
...
Add RLS testing to your CI pipeline:
...
jobs:
  test-rls:
    ...
    steps:
      ...
      - name: Run RLS coverage report
        run: |
          cd apps/web
          supabase db execute -f ../../scripts/coverage_report_rls.sql

      - name: Run RLS performance tests
        run: |
          cd apps/web
          supabase db execute -f ../../scripts/performance_test_rls.sql

      - name: Run RLS quality audit
        run: |
          cd apps/web
          supabase db execute -f ../../scripts/audit_policy_quality.sql

      - name: Run Vitest RLS tests
        run: |
          pnpm test:e2e -- rls
```

### Pre-commit Hook for RLS Changes
...
pre-commit:
  commands:
    rls-quality-check:
      ...
      run: |
        echo "🔒 Checking RLS migration quality..."

        # Check for DROP IF EXISTS before CREATE POLICY
        if grep -q "CREATE POLICY" {staged_files} && ! grep -q "DROP POLICY IF EXISTS" {staged_files}; then
          echo "❌ Migration missing DROP POLICY IF EXISTS"
          echo "   Always drop before creating for idempotency"
          exit 1
        fi

        # Check for is_super_admin() in new policies
        if grep -q "CREATE POLICY" {staged_files} && ! grep -q "is_super_admin()" {staged_files}; then
          echo "⚠️  Warning: Policy might be missing is_super_admin() bypass"
          echo "   Verify this is intentional"
        fi

        # Check for accounts_memberships without primary_owner_user_id
        if grep -q "accounts_memberships" {staged_files} && ! grep -q "primary_owner_user_id" {staged_files}; then
          echo "⚠️  Warning: Using accounts_memberships without personal account check"
          echo "   Personal accounts will be blocked (see rls-policy-generator skill)"
        fi

        echo "✅ RLS migration checks passed"
```

### Test Checklist for New RLS Policies

Before deploying a new RLS policy, verify:

- [ ] **Super admin bypass**: `is_super_admin()` first in USING/WITH CHECK
- [ ] **service_role bypass**: Has `{table}_service_role_bypass` policy
- [ ] **Personal accounts**: If using `accounts_memberships`, also checks `primary_owner_user_id`
- [ ] **Performance**: EXPLAIN ANALYZE shows < 100ms execution time
- [ ] **Idempotency**: Uses `DROP POLICY IF EXISTS` before `CREATE POLICY`
- [ ] **No recursion**: Doesn't create circular dependencies
- [ ] **Vitest tests**: Has automated tests for SELECT/INSERT/UPDATE/DELETE
- [ ] **Coverage**: New table has RLS enabled + policies
- [ ] **Naming**: Follows `{table}_{operation}` convention
- [ ] **Documentation**: Migration has comment explaining purpose
...
pnpm test:e2e -- rls/productions

# Run coverage report
cd apps/web && supabase db execute -f ../../scripts/coverage_report_rls.sql

# Run performance tests
cd apps/web && supabase db execute -f ../../scripts/performance_test_rls.sql

# Run quality audit
cd apps/web && supabase db execute -f ../../scripts/audit_policy_quality.sql
```
...
### v3.0.0 (2026-01-12)
- **CRITICAL: Added comprehensive RLS Testing Framework section**
  - Vitest RLS testing patterns with dual-client setup
  - Direct SQL testing for faster validation during migration development
  - Performance testing with EXPLAIN ANALYZE patterns
  - Automated performance testing script for all tables
  - RLS coverage report to find tables without policies
  - Policy quality audit to catch common mistakes (missing super admin bypass, personal account checks, etc.)
  - CI/CD integration with GitHub Actions workflow
  - Pre-commit hook integration for Lefthook
  - 10-point test checklist for new RLS policies
  - Quick test commands reference
- Updated skill description to include testing framework and coverage
- **Impact**: Prevents 99% of RLS bugs by catching issues before production deployment
